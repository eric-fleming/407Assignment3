 ...
 *---	----	----	----	----	----	----	----	----	---*
 *---									---*
 *---	Version 1.0		2017 August 1		Joseph Phillips	---*
 *---									---*
 *-------------------------------------------------------------------------*/
				...
				)
{
  ...
  //(1)
  if(argc < 2){
      fprintf(stderr,"No input string\t /filepath\n");
      exit(EXIT_FAILURE);
  }
  //(2)
  strncpy(textSpace,argv[1],textSpaceLen);
  //(3)
  char* newlinechar;
  newlinechar = strchr(textSpace,'\n');
  if(newlinechar != NULL){
      *newlinechar = '\0';
  }
  //(4)
				...
				)
{
  ...
  //  YOUR CODE HERE
  if(linePtr == '\0'){
      struct	DirEntryName* myDirEntryNamePtr;
      myDirEntryNamePtr = (struct DirEntryName*)malloc(sizeof(struct DirEntryName));
  }
  ...
  //  YOUR CODE HERE
  while(isLegalDirEntryChar(*linePtr) != 0){
      linePtr++;
  }
  ...
  //  YOUR CODE HERE
  struct DirEntryName* currentDirPtr = (struct DirEntryName*)malloc(sizeof(struct DirEntryName));
  int dirlength = linePtr-entryNamePtr+1;

  char* dirname = (char*)malloc(dirlength);
  strncpy(dirname,entryNamePtr,dirlength);

  currentDirPtr->name_ = dirname;
  ...
  //  YOUR CODE HERE
  if(*linePtr == '/'){
      currentDirPtr->nextPtr_ = parseRestOfPath(linePtr);
  }
  if(*linePtr == '\0'){
      return NULL;
  }
...
{
  //  YOUR CODE HERE
  //I assume some sort of recursive call to destroy the last leaf and work back to the root?
  struct DirEntryName* child = pathNamePtr->dirEntryNamePtr_;
  free(pathNamePtr);
  if(child == NULL){
      exit(EXIT_SUCCESS);
  }
  else{
      struct DirEntryName* grandchild = child->nextPtr_;
      if(grandchild == NULL){
          free(child);
      }
      while(grandchild != NULL){
          struct DirEntryName* descendant = grandchild->nextPtr_;
          free(grandchild);
